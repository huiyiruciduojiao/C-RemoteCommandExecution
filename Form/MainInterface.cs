...
namespace TCPConsole {
    public partial class MainInterface : Form {
        ...
        //创建远程文件连接对象
        RemoteFileTools remoteFileTools = new RemoteFileTools();

        //运行时代理域名解析地址以及端口
        public static string RuntimeAgentIP = "";
        public static int RuntimeAgentPort = 0;
        ...
        #region 菜单工具按钮处理
        //浏览远程文件按钮
        private void RemoteFileRFToolStripMenuItem_Click(object sender, EventArgs e) {
            ShowRemoteFile showRemoteFile = new ShowRemoteFile(remoteFileTools, commandExecutionConfiguration.Address, 7979, commandExecutionConfiguration, ref commandExecutor);
            showRemoteFile.Show();
        }
        //代理连接按钮
        private void ProxyConnectionToolStripMenuItem_Click(object sender, EventArgs e) {
            bool ProxyConnection = ProxyConnectionToolStripMenuItem.Checked;
            ProxyConnectionToolStripMenuItem.Checked = !ProxyConnection;
            ConfigurationTools c = new ConfigurationTools("./config.ini");
            string agentHost = c.GetSetting("Defalut", "AgentHost");
            Console.WriteLine(agentHost);
            int agentPort = Convert.ToInt32(c.GetSetting("Defalut", "AgentPort"));
            if (!ProxyConnection && tools.IsDomain(agentHost)) {
                richResultsOfEnforcementAppendText(new RichTextBoxStyle("代理配置正在进行域名解析:", Color.Yellow, true));
                Task t = new Task(() => {
                    IPAddress[] iPAddresses = tools.DomainNameResolution(agentHost);
                    if (iPAddresses != null && iPAddresses.Length > 0) {
                        RuntimeAgentIP = iPAddresses[0].ToString();
                        SyncContext.Post(richResultsOfEnforcementAppendText, new RichTextBoxStyle("代理配置域名解析完毕:" + RuntimeAgentIP, Color.Black, true));
                    } else {
                        ProxyConnectionToolStripMenuItem.Checked = false;
                        ProxyConnection = false;
                        SyncContext.Post(richResultsOfEnforcementAppendText, new RichTextBoxStyle("代理配置域名解析失败", Color.Red, true));
                        SyncContext.Post(richResultsOfEnforcementAppendText, new RichTextBoxStyle("代理连接状态:已关闭代理连接", Color.Red, true));
                    }
                });
                t.Start();
            } else if (tools.IsItIP(agentHost)) {
                RuntimeAgentIP = agentHost;
            }
            RuntimeAgentPort = agentPort;
            richResultsOfEnforcementAppendText(new RichTextBoxStyle("代理连接状态:" + (!ProxyConnection ? "已开启代理连接" : "已关闭代理连接"), !ProxyConnection ? Color.Green : Color.Red, true));

        }
        //Ping测试点击事件
        private void LocPingToolStripMenuItem_Click(object sender, EventArgs e) {
            if (isExit) {
                commandLocal = new Command();
                reSetCommandLocal();
            }

            commandExecutor.LocalExecution("ping " + commandExecutionConfiguration.Address, commandLocal);
        }
        #endregion
        #region 系统托盘事件处理
        //系统托盘——打开主界面
